...
Timer* timers = (Timer*) 0x4000100;

void waitVB(unsigned int waitTime){
	unsigned int start = vBlanks;
	while(start + waitTime > vBlanks){
		waitForVblank();
	}
...
void drawSprite(Sprite *sprite) {
	*(sprite->spriteAttr0) = (sprite->arow & ROWMASK) | (*(sprite->spriteAttr0) & ~ROWMASK) |ATTR0_8BPP;
	*(sprite->spriteAttr1) = (sprite->acol & COLMASK) | (*(sprite->spriteAttr1) & ~COLMASK);
}

int checkCollision(Sprite *s1, Sprite *s2){
	return (s1->acol <= s2->acol + 8 && s1->acol >= s2->acol -8) &&
			(s1->arow <= s2->arow + 8 && s1->arow >= s2->arow - 8);
}
